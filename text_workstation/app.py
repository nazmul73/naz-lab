...
def clear_mode_state(mode: str) -> None:
    for suffix in ["input", "output", "saved_path", "job_path", "status", "input_widget"]:
        key = state_key(mode, suffix)
        if key in st.session_state:
            del st.session_state[key]


def sync_mode_input(mode: str) -> None:
    input_key = state_key(mode, "input")
    widget_key = state_key(mode, "input_widget")
    st.session_state[input_key] = st.session_state.get(widget_key, "")
...
def render_writer_mode(mode: str, model: str) -> None:
    ...
    if widget_key not in st.session_state:
        st.session_state[widget_key] = st.session_state[input_key]
    ...
    if generate_clicked:
        ...
        try:
            ...
            st.session_state[status_key] = "Timed out. On CPU runtime this model may be too slow. Restart Ollama or switch to T4 GPU."
    ...
    if st.session_state.get(status_key) and st.session_state[status_key] != "Ready":
...
def main() -> None:
    ...
    st.caption("Phase 1.2 Hotfix: General Chat, Free Writer, Re-writer, Story, Script, Caption, Prompt Improver")
