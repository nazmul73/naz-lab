...
- template-first fallback for Bangla content modes
- text output save to Drive
...
MODE_CONFIG = {
    "Free Writer": {"output_dir": TEXT_OUTPUTS, "prefix": "free_writer"},
    "Story Writer": {"output_dir": TEXT_OUTPUTS, "prefix": "story_writer"},
    "Viral Script Writer": {"output_dir": SCRIPT_OUTPUTS, "prefix": "viral_script_writer"},
    "Caption Writer": {"output_dir": TEXT_OUTPUTS, "prefix": "caption_writer"},
    "Prompt Improver": {"output_dir": IMAGE_PROMPTS, "prefix": "prompt_improver"},
}
...
def render_builder() -> None:
    ...
    with col1:
        ...
        mode = st.selectbox("Mode", list(MODE_CONFIG.keys()), index=1)
    with col2:
    ...
    template_first = st.checkbox("Template-first for Bangla output", value=True, help="Best for stable CPU tests. Model output remains available when quality is high.")
...
def main() -> None:
    ...
    st.caption("Phase 1.10 — qwen2.5:1.5b, Bangla Safe Mode default, template-first stable CPU output, Drive save, Image Job JSON handoff")
